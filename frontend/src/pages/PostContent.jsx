...
export default function PostContent() {
  const { upload, startUpload, draft, patchDraft } = useUpload();

  const videoFile    = upload?.file    || null;
  const videoPreview = upload?.preview || null;
  const videoUrl     = upload?.videoUrl || null;
  ...
  // Ephemeral state — losing on nav is fine
  ...
  const goToAI = async () => {
    ...
    if (frames.length === 0 && videoRef.current) {
      try { const f = await extractFrames(videoRef.current, 6); set("frames")(f); } catch {}
    }
  };
  ...
  if (done) return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:400, gap:20, textAlign:"center" }}>
      ...
      <h2 style={{ fontSize:26, fontWeight:800, color:"#F5F5FF" }}>{postNow ? "Content posted!" : "Content scheduled!"}</h2>
      ...
      <button onClick={() => setDone(false)} className="btn-primary" style={{ padding:"12px 32px", fontSize:14 }}>Post Another →</button>
    </div>
  );
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                <div style={{ flex:1, fontSize:14, color:"#B0B0D0", fontWeight:600 }}>
                  {uploading ? `Uploading… ${upload.progress}%` : `✓ ${videoFile?.name}`}
                </div>
                {!uploading && (
              ...
              {upload?.status === "error" && (
                <div style={{ fontSize:13, color:"#EF4444" }}>⚠ {upload.error} — <span style={{ cursor:"pointer", textDecoration:"underline" }} onClick={() => startUpload(videoFile)}>Retry</span></div>
              )}
              <div style={{ display:"flex", gap:12, justifyContent:"flex-end", marginTop:8 }}>
                <button onClick={goToAI} className="btn-ghost" style={{ padding:"11px 22px", fontSize:14 }} disabled={uploading}>Skip Editing →</button>
                <button onClick={() => set("step")(1)} className="btn-primary" style={{ padding:"11px 28px", fontSize:14 }} disabled={uploading}>Edit & Clip →</button>
              </div>
            </div>
          )}
        </div>
      ...
      {step === 2 && (
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          ...
          <div className="card" style={{ padding:24, display:"flex", flexDirection:"column", gap:14 }}>
            ...
            {aiLoading && (
              <div style={{ fontSize:13, color:"#9090B8", padding:"10px 14px", background:"#0A0A22", borderRadius:8, border:"1px solid #1E1E42", display:"flex", alignItems:"center", gap:8 }}>
                <span style={{ animation:"spin 1s linear infinite", display:"inline-block" }}>◌</span>
                AI is generating your title, description and hashtags…
              </div>
            )}
            <div>
